...
- **Anthropic SDK (@anthropic-ai/sdk)** — Node.js向けAnthropic公式SDK（名刺バトラー）
- **Next.js App Router API Routes（AI連携）** — サーバーサイドでClaude Vision APIを呼び出すAPIエンドポイント構築（名刺バトラー）
...
| 台灣生成新聞 | Next.js, TypeScript, Claude API (Haiku), Upstash Redis, Resend API, Together AI, Vercel Cron Jobs, RSS解析(11フィード) | 台湾11メディアRSSからAIが毎朝キュレーション。TAIEX株価・台北天気ヘッダー、6コーナー構成（トレンド/カフェ＆グルメ/ビューティー＆ブランド/SNSバズ/台湾人が見ている日本/台湾文化史）、新聞風レイアウト（2段組・カードグリッド・引用スタイル混在）、メール配信対応（配信解除リンク付き）。管理画面：ダッシュボード（URLコピー・配信状況・号数リセット）・読者管理・ページプレビュー |
